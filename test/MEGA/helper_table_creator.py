...
def build_profiles(df: pd.DataFrame,
    ...
    def pick_row(g: pd.DataFrame) -> pd.Series:
        ...
        """
       If there are multiple rows with the same profile, system, and marker, pick the representative row.
       Pick non-null result else highest count else lowest error
        """

        g = g.copy()
        g["_has_result"] = g["RESULT"].notna()
        g["_count"] = (pd.to_numeric(g["COUNT"], errors="coerce").fillna(-1) if "COUNT" in g.columns else -1)
        g["_err"] = (pd.to_numeric(g["RESULT_ERROR"], errors="coerce").fillna(np.inf) if "RESULT_ERROR" in g.columns else np.inf)

        g = g.sort_values(["_has_result", "_count", "_err"], ascending=[False, False, True])

        return g.iloc[0]
    
    chosen = (
        d.groupby(["PROFILE_KEY", "SYSTEM_SHORT", "MARKER_SHORT"], dropna=False)
        .apply(pick_row)
...
def write_excel_chunk(output_xlsx: str, profiles: List[Profile], canonical: "OrderedDict[str, List[str]]", profile_type: str, control_csv: Optional[str], sigma_csv: Optional[str], *,
    ...
    for data in sorted_profiles:
        ...
        for j in range(len(pairs)):
            cell = WriteOnlyCell(ws_data, value=(None if np.isnan(vals[j]) else vnums[j]))
    ...
    #Envelope sheet
    if from_export and not control_export_csv.empty and assay_ids is not None:
        ...
        helper_envelope.run_helper_envelope(control_csv=control_csv, sigma_csv=sigma_csv, readout_names_in_order=names, workbook_or_path=wb, confidence=confidence, data_type=data_type, csv_sep=csv_sep, envelope_type_label="Sig", save_path=None, envelope_source="hit")
...
def run_helper_table_creator(
        ...
        control_csv = CTRL_DIR / "NONPROD_SingleRepeatControls_Cleaned.csv", sigma_csv = CTRL_DIR / "NONPROD_SingleRepeatControls_sigma_Cleaned.csv", from_export: bool = True, assay_ids_csv= CTRL_DIR / "envelope_assay_list_20260311.csv",
        concentration: float = 0.1, exclude_visual: bool = True,):
    ...
    if total_rows > excel_max_rows:
        ...
        for part in range(n_parts):
            ...
            out           = write_excel_chunk(part_path, prof_slice, canonical, pt, control_csv, sigma_csv, from_export=from_export, control_export_csv=control_export_csv, assay_ids=assay_ids, confidence=confidence, data_type=profile_type.capitalize(), csv_sep=",", concentration=concentration, exclude_visual=exclude_visual)
            output_files.append(out)
    else:
        out = write_excel_chunk(output_xlsx, profiles, canonical, pt, control_csv, sigma_csv, from_export=from_export, control_export_csv=control_export_csv, assay_ids=assay_ids, confidence=confidence, data_type=profile_type.capitalize(), csv_sep=",", concentration=concentration, exclude_visual=exclude_visual)
        output_files.append(out)
