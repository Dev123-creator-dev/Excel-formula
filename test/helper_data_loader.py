...
FALLBACK_CSV    = BASE_DIR / "data" / "snapshots" / "snapshot_2026_05_20.parquet"
FALLBACK_DMSO   = BASE_DIR / "data" / "snapshots" / "snapshot_dmso_2026_05_20.parquet"
FALLBACK_ENC    = "utf-8"
KEEP_DAYS       = 7
DENOM           = 148
...
def load_panel() -> pd.DataFrame:
    ...
    panel["SYSTEM"] = panel["SYSTEM"].astype(str).str.strip().str.upper()
    panel["MARKER"] = panel["MARKER"].astype(str).str.strip().str.upper()
...
def _dated_xlsx(stem: str, d: Optional[date] = None) -> Path:
    d = d or date.today()
    return SNAPSHOT_DIR / f"{stem}_{d.strftime('%Y_%m_%d')}.xlsx"
...
def _pull_from_db() -> pd.DataFrame:
    ...
    if (dmso_df is None or dmso_df.empty) and FALLBACK_DMSO.exists():
        logger.error("Using fallback DMSO file")
        dmso_df = pd.read_parquet(FALLBACK_DMSO)

    if df is None or df.empty:
        raise RuntimeError(f"No database query available and no fallback CSV found at {FALLBACK_CSV}")
    
    df_norm         = _normalize_keys(df)
    dmso_df_norm    = _normalize_keys(dmso_df)
    
    return df_norm, dmso_df_norm
...
def refresh_daily_snapshot(denom: int = DENOM) -> None:
    ...
    helper_table_creator.run_helper_table_creator(df, _dated_xlsx("MEGA_Export"))
