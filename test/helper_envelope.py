...
def _convert_header(col: object) -> str:
    """
    Convert a raw control/sigma-file column header ('Long_System_Name:Marker') to the
    short-code form ('SYS:Marker') used everywhere else in this app, so pre-built
    control/sigma CSVs (which use long system names, matching the Java Viewer's own
    export format) can be matched against short-code readout requests.
    """
    col = _normalize_sysmrk(col)
    if ":" not in col:
        return col
    system_long, marker = col.split(":", 1)
    return f"{sys_converter(system_long)}:{mrk_converter(marker)}"
...
def _bisect_multiplier(sigma_arr: np.ndarray, ctrl_matrix: np.ndarray, confidence: float) -> float:
    """
    Direct translation of EnvelopeService.calculateHitEnvelope / getPercentInEnvelope
    (see BioMAPViewer/src/com/bioseek/rview/service/EnvelopeService.java).

    Finds the smallest multiplier M such that at least `confidence` fraction of
    control-matrix rows fall within M*sigma for every readout (column) simultaneously
    — a joint/simultaneous containment search across all readouts at once, not a
    per-readout marginal quantile.
    """
    def _percent_in_envelope(multiplier: float) -> float:
        big_sigmas = sigma_arr * multiplier
        in_envelope = np.all(ctrl_matrix <= big_sigmas[np.newaxis, :], axis=1)
        return float(in_envelope.mean())

    n_rows = ctrl_matrix.shape[0]
    max_sig_level = 1.0 - 1.0 / n_rows
    target_confidence = min(confidence, max_sig_level)

    m_high   = 10.0
    pct_high = _percent_in_envelope(m_high)
    while pct_high < target_confidence:
        m_high  *= 2
        pct_high = _percent_in_envelope(m_high)

    m_low      = 0.0
    error_term = max(1.0 / n_rows, 0.00001)
    while not ((pct_high - target_confidence) >= 0 and (pct_high - target_confidence) <= error_term) \
            and (m_high - m_low) > 0.0000001:
        m_mid   = (m_low + m_high) / 2
        pct_mid = _percent_in_envelope(m_mid)
        if pct_mid < target_confidence:
            m_low = m_mid
        else:
            m_high   = m_mid
            pct_high = pct_mid
...
class EnvelopeService:
    ...
    # ------------------------------------------------------------------
    # Path 0: pre-computed Significance Envelope table (pure lookup)
    # Translates: EnvelopeService.calculateSigEnvelope(EnvelopeModel) — the
    # Significance Envelope is never computed live in the real app; it's written
    # once by the offline UpdateControlDataService batch job into the database
    # table reload.significance_envelope, then just looked up at view/export time.
    # ------------------------------------------------------------------

    def lookup_significance_table(self, model: EnvelopeModel, table_csv: str,
                                   profile_type_id: int) -> EnvelopeModel:
        df = pd.read_csv(table_csv)
        rows = df[(df["PROFILE_TYPE_ID"] == profile_type_id)
                  & (df["PERCENT_CONFIDENCE"] == model.confidence)]
        value_by_norm = {_normalize_sysmrk(sr): float(v)
                          for sr, v in zip(rows["SYS_READ"], rows["env_value"])}

        sig_map: Dict[str, float] = {}
        for sr in model.system_marker_list:
            v = value_by_norm.get(_normalize_sysmrk(sr))
            if v is not None:
                sig_map[sr] = v

        model.significance_envelope_map = sig_map
        model.hit_envelope_map          = dict(sig_map)  # same map; Excel writer selects by envelope_source
        model.system_marker_map         = _build_system_marker_map(list(sig_map.keys()))
        return model
    ...
    def envelope_service_from_export(
        ...
        *** 24March2026: Removed Phases 2 & 3 and set multiplier based on z-scole a la Java code ***
        ...
        #### PHASE 2: build global synthetic control matrix + per-readout significance envelope
        # Direct translation of UpdateControlDataService.service()'s main per-readout loop:
        #   stddev = BSKStatistics.stddev(all_ratios)
        #   double mean = 0;                                  <- HARDCODED 0 (see _fill_by_norm_dist docstring)
        #   filled = fillByNormDist(all_ratios, mean, stddev, matrix_size)
        #   sigEnvValues = calSigEnv(filled)                  <- percentile of |filled| at `confidence`
        #   fillMatrix(ctrl_matrix, filled, col)               <- Java's file round-trip folds to Math.abs()
        matrix_size = (3 * MAX_SCREENPROFILE_NUM) if not is_screening else MAX_SCREENPROFILE_NUM

        sigma_arr   = np.empty(len(common_readouts), dtype=float)
        sig_env_map: Dict[str, float] = {}
        ctrl_matrix = np.empty((matrix_size, len(common_readouts)), dtype=float)
        ...
        for j, sr in enumerate(common_readouts):
            ...
            filled = _fill_by_norm_dist(all_ratios, 0.0, sigma_j, matrix_size, rng)
            pos = np.abs(filled)
            ctrl_matrix[:, j] = pos

            sig_env_map[sr] = float(java_percentile(pos, confidence)) if sigma_j > 0 else float("nan")

        # ---- PHASE 3: global bisection for the hit-envelope multiplier M ----
        # Direct translation of EnvelopeService.calculateHitEnvelope / getPercentInEnvelope:
        # find the smallest M such that at least `confidence` fraction of whole synthetic
        # profiles (every readout at once) fall within M*sigma for every readout simultaneously.
        def _percent_in_envelope(multiplier: float) -> float:
            big_sigmas = sigma_arr * multiplier
            in_envelope = np.all(ctrl_matrix <= big_sigmas[np.newaxis, :], axis=1)
            return float(in_envelope.mean())
        ...
        the_multiplier = m_high
        #print(f"[DEBUG] Phase 3 bisection: M={the_multiplier:.6f}, achieved={pct_high:.4f}, target={target_confidence:.4f}")

        hit_map_inner: Dict[str, float] = {}
        for j, sr in enumerate(common_readouts):
            hit_map_inner[sr] = float(sigma_arr[j] * the_multiplier) if sigma_arr[j] > 0 else float("nan")
    ...
    def _read_control_data(self, control_csv: str, requested: List[str],
        ...
        # File headers use long "System_Long_Name:Marker" form (e.g. from a raw DB/Viewer
        # export); requested readouts use short codes (e.g. "3C:Marker"). Convert headers
        # via the same sys_converter/mrk_converter used elsewhere before matching.
        hdr_norm   = {c: _convert_header(c) for c in df.columns}
    ...
    def _read_sigmas(self, sigma_csv: str, readouts: List[str],
        ...
        cols_norm = {c: _convert_header(c) for c in df.columns}
        row       = df.iloc[0]
...
def write_envelope_sheet_xlsx(
    ...
    for idx, r in enumerate(readout_names_in_order, start=2):
        ...
        if val is None:
            continue
        try:
            rounded_val = round(float(val), 5)
            ws.cell(row=2, column=idx, value=rounded_val)
            ws.cell(row=3, column=idx, value=-rounded_val)
...
def run_helper_envelope_from_table(
    table_csv: str,
    profile_type_id: int,
    readout_names_in_order: List[str],
    workbook_or_path: Union[str, Workbook],
    ...
    save_path: Union[str, None] = None,
    envelope_source: str        = "significance",
) -> Workbook:
    """
    Path 0 wrapper: look up pre-computed Significance Envelope values directly from
    a dump of the reload.significance_envelope table, instead of computing anything.
    """
    model = EnvelopeModel()
    ...
    model = EnvelopeService().lookup_significance_table(model, table_csv, profile_type_id)
